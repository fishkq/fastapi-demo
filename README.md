...
services:
  db:
    ...
    deploy:
      placement:
        constraints:
          - node.labels.fastapi-demo-com.app-db-data == true
```
...
```bash
docker node update --label-add fastapi-demo-com.app-db-data=true dog.example.com
```

* Then you need to do the same for each stack version you have. For example, for staging you could do:

```bash
docker node update --label-add stag-fastapi-demo-com.app-db-data=true cat.example.com
```
...
PGAdmin: https://pgadmin.fastapi-demo.com
...
PGAdmin: https://pgadmin.stag.fastapi-demo.com
...
PGAdmin: http://dev.fastapi-demo.com:5050
